class Properties {
    static icon = "https://waa.ai/JEwK.png";
    static color = "purple";
    static fullname = "Kurumi Mimino";
    static alter_ego = "Milky Rose";
    static hint_chiguhaguu = "The blue rose is my secret emblem! <x>!";
    static total = 0;
    static series = "yes5gogo";
}

class Avatar {
    static normal = {
        icon:"https://waa.ai/JEwK.png",
        name:"Milky Rose",
        transform_quotes1:"Skyrose Translate!",
        transform_quotes2:"The blue rose is my secret emblem! Milky Rose!",
        special_attack:"Metal Blizzard",
        img_special_attack:"https://cdn.discordapp.com/attachments/793380840255389716/817776335572172880/unknown.png",
        img_transformation:"https://cdn.discordapp.com/attachments/793380840255389716/826556276469137418/image0.gif",
        skill:{
            passive:{}
        }
    }
}

module.exports = {
    Properties, Avatar
}