class Properties {
    static icon = "https://waa.ai/JEwt.png";
    static color = "yellow";
    static fullname = "Urara Kasugano";
    static alter_ego = "Cure Lemonade";
    static hint_chiguhaguu = "The effervescence of bursting lemon, <x>!";
    static total = 0;
    static series = "yes5gogo";
}

class Avatar {
    static normal = {
        icon:"https://waa.ai/JEwt.png",
        name:"Cure Lemonade",
        transform_quotes1:"Pretty Cure Metamorphose!",
        transform_quotes2:"The effervescence of bursting lemon, Cure Lemonade!",
        special_attack:"Prism Chain",
        img_special_attack:"https://cdn.discordapp.com/attachments/793380077173735424/817776088595300452/unknown.png",
        img_transformation:"https://cdn.discordapp.com/attachments/793380077173735424/822044339370065941/image0.gif",
        skill:{
            passive:{}
        }
    }
}

module.exports = {
    Properties, Avatar
}