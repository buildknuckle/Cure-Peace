...
class Gachapon {
    static dailyRollsCardData = {
        1:[], 5:[], 6:[],
    };

    static tropicalCatchData = {
        1:[], 5:[], 6:[],
    }
    ...
    static async init(){
        ...
        for(var i=0;i<results.length;i++){
            var card = results[i];
            this.dailyRollsCardData[card[Card.columns.rarity]].push(card);
        }
        ...
        for(var i=0;i<results.length;i++){
            var card = results[i];
            this.tropicalCatchData[card[Card.columns.rarity]].push(card);
        }

    }

    static infoDaily(){
        var arrSeriesDaily = ["max_heart","splash_star","yes5gogo","fresh","heartcatch","suite","smile","dokidoki","happiness_charge","go_princess","mahou_tsukai","kirakira","hugtto"];
        var arrSeriesDailyLimited = ["max_heart","splash_star","yes5gogo","fresh"];
        
        //rarity 1 & 5
        var seriesTextDailyNormal = ``;
        for(var key in SPack){
            if(arrSeriesDaily.includes(key)){
        ...
        for(var key in SPack){
            if(arrSeriesDailyLimited.includes(key)){
                var series = new Series(key);
        ...
        **Overview**
        ...
        • 1x (Costs: ${Gachapon.cost.daily[1]} ${Currency.jewel.emoji}): will roll 1x card drop
        • 5x (Costs: ${Gachapon.cost.daily[5]} ${Currency.jewel.emoji}): will roll 5x card drop 

        ─────────────────
        **Series lineup**
        ─────────────────
        The following series lineup will appear on this gachapon:
        ${Card.emoji.rarity(1)}__**1 (drop rates: 85%)**__
        ${seriesTextDailyNormal}

        ${Card.emoji.rarity(5)}__**5 (drop rates: 10%)**__
        ${seriesTextDailyNormal}

        ${Card.emoji.rarity(6)}__**6 (limited) (drop rates: 5%)**__
        ...
        **Notes**
        ...
        return Embed.builder(txtInfo, 
            ...
            title:`Daily Gachapon`
        })
    }

    static infoTropicalCatch(){
        ...
        **Overview**
        ...
        • 1x (Costs: ${Gachapon.cost.tropical[1]} ${Currency.jewel.emoji}): will roll 1x card drop
        • 3x (Costs: ${Gachapon.cost.tropical[3]} ${Currency.jewel.emoji}): will roll 3x card drop 
...
class Listener extends require("./data/Listener") {
    ...
    async dailyRoll(roll){//print user status menu
        ...
        var jewelCost = Gachapon.cost.daily[roll];
        if(user.Currency.jewel<jewelCost){ //validation: check for jewel
            return this.interaction.reply(Validation.notEnoughJewel(this.discordUser, jewelCost));
        } else if(userGacha.hasDailyGacha()){//validation: check if already use daily gacha
        ...
        userGacha.setTodayDailyGachaDate();
        ...
        for(var i=0;i<roll;i++){
            ...
            var rndRarity;
            if(rnd<=85){ rndRarity = 1; }//roll 1*
            else if(rnd<=95){ rndRarity = 5;}//roll 5*
            else { rndRarity = 6; }//roll 6* premium

            var rndCardData = GlobalFunctions.randomProperty(Gachapon.dailyRollsCardData[rndRarity]);
