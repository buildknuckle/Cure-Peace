...
class Spawner {
    ...
    static async spawnQuiz(guildId){
        ...
        var objEmbed = new MessageEmbed({
            ...
            image: {
                url:Properties.imgSet.mofu.peek
            }
        });
...
class EventListener {
    static spawnValidation(objUserData, userStatusData, guildData, spawnType, subSpawnType=""){
        if(guildData[DBM_Guild_Data.columns.spawn_type]==null){
            ...
            return GEmbed.errorMini(`:x: This command is not available for ${Spawner.spawnDataType[spawnType].name} spawn this time.`,objUserData,true);
            ...
            if("type" in parsedType){
                if(parsedType["type"]!=subSpawnType){
                    return GEmbed.errorMini(`:x: This command is not available for different type of ${Spawner.spawnDataType[spawnType].name} spawn this time.`,objUserData,true);
                }
            }
        }
    ...
    static async captureNormal(objUserData, guildId, isBoostCaptured = false){
        ...
        if(isBoostCaptured){//use boost command
            ...
            if(userStatusData[DBM_User_Data.columns.peace_point]<boostCost)
                return {embeds:[Embed.notifNotEnoughBoostPoint(boostCost, objUserData)], ephemeral:true};
            else{
                rateCatch = 100;
                updateData[DBM_User_Data.columns.peace_point] = -boostCost;
        ...
        if(GlobalFunctions.randomNumber(0,100)<=rateCatch){//success
            ...
            var inventoryData = await Inventory.getData(userId, cardData[DBM_Card_Data.columns.id_card]);
            var updatedStock = await Inventory.updateStockParam(userId, id, inventoryData, qty);
            var total = await Inventory.getPackTotal(userId, pack);
    ...
    static async captureJankenpon(objUserData, guildId, choice){
        ...
        if(jankenponData[rndJankenpon].choiceResults[choice]){//results: win
            ...
            var inventoryData = await Inventory.getData(userId, id);
            var updatedStock = await Inventory.updateStockParam(userId, id, inventoryData, qty);
            var total = await Inventory.getPackTotal(userId, pack);
